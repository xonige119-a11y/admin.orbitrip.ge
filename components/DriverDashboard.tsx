...
const DriverDashboard: React.FC<DriverDashboardProps> = ({ 
  ...
  // Author Tour State
  const [isAuthorTourModalOpen, setIsAuthorTourModalOpen] = useState(false);
  const [editingAuthorTour, setEditingAuthorTour] = useState<Partial<Tour>>({});
  ...
  // Only show AUTHOR tours created by this driver (by convention, ID could contain driver ID or separate field, 
  // but for simplicity we filter by category AUTHOR and title including driver name or assume shared ownership for now if no auth field)
  // For this version, let's assume drivers can see ALL AUTHOR tours they created if we add an ownerId to Tour type later.
  // Currently, Tour type doesn't have ownerId. We will filter by category 'AUTHOR' and maybe assume trust.
  // Ideally: filter by a new field ownerId. For now, let's just show all AUTHOR tours to all drivers as a shared pool or filter by title convention.
  // LET'S ADD A TEMPORARY CONVENTION: Driver Name in Title for filtering? No, let's just show all AUTHOR tours.
  const myAuthorTours = useMemo(() => {
      // In a real app, Tour should have ownerId. Here we filter all AUTHOR category tours.
      // If we want specific ownership, we'd need schema migration.
      // For MVP: Show ALL tours where category is AUTHOR.
      return tours.filter(t => t.category === 'AUTHOR');
  }, [tours]);
  ...
  // Author Tour Handlers
  const openEditAuthorTour = (tour?: Tour) => {
      setIsUploadingTour(false);
      if (tour) {
          setEditingAuthorTour(JSON.parse(JSON.stringify(tour))); 
      } else {
          setEditingAuthorTour({
              id: `tour-auth-${Date.now()}`,
              titleEn: `My Author Tour (${currentProfile.name})`, titleRu: `Мой Авторский Тур (${currentProfile.name})`,
              descriptionEn: '', descriptionRu: '',
              price: 'From 200 GEL', basePrice: 200, duration: '6 Hours', category: 'AUTHOR',
              image: '', highlightsEn: [], highlightsRu: [], routeStops: [],
              priceOptions: [{ vehicle: currentProfile.vehicleType, price: "200 GEL", guests: `1-${currentProfile.maxPassengers}` }],
              rating: 5, reviews: []
          });
      }
      setIsAuthorTourModalOpen(true);
  };

  const saveAuthorTour = (e: React.FormEvent) => {
      e.preventDefault();
      const tourToSave = editingAuthorTour as Tour;
      
      // Enforce AUTHOR category
      tourToSave.category = 'AUTHOR';
      
      const exists = tours.find(t => t.id === tourToSave.id);
      exists ? onUpdateTour(tourToSave) : onAddTour(tourToSave);
      setIsAuthorTourModalOpen(false);
  };

  const handleAuthorTourImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      const file = e.target.files?.[0];
      if (!file || !editingAuthorTour.id) return;
      setIsUploadingTour(true);
      try {
          const url = await storageService.uploadTourImage(file, editingAuthorTour.id);
          if (url) setEditingAuthorTour(prev => ({ ...prev, image: url }));
      } catch (err: any) { alert(`Upload Failed: ${err.message}`); } 
      finally { setIsUploadingTour(false); }
  };
  
  const handleAuthorTourArrayInput = (field: keyof Tour, value: string) => {
      setEditingAuthorTour(prev => ({
          ...prev,
    ...
    <div className="flex flex-col h-screen bg-slate-50 md:flex-row font-sans overflow-hidden">
        ...
        <div className={`fixed inset-y-0 left-0 z-20 bg-slate-900 text-white transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 w-72 flex flex-col ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} pt-16 md:pt-0`}>
            ...
            <nav className="flex-1 p-4 space-y-2">
                {[
                    ...
                    { id: 'MY_TOURS', icon: '⭐', label: "My Author's Tours" },
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50 p-4 md:p-8">
            <div className="max-w-6xl mx-auto">
                ...
                {/* --- TAB: MY AUTHOR'S TOURS --- */}
                {activeTab === 'MY_TOURS' && (
                     <div className="space-y-6 animate-fadeIn">
                         <div className="flex justify-between items-center mb-6">
                             <div>
                                 <h2 className="text-2xl font-bold text-gray-800">My Author's Tours / ჩემი საავტორო ტურები</h2>
                                 <p className="text-sm text-gray-500">Create unique trips that only YOU can offer.</p>
                             </div>
                             <button onClick={() => openEditAuthorTour()} className="bg-indigo-600 text-white px-4 py-2 rounded-lg font-bold shadow hover:bg-indigo-700 transition">+ Create Tour</button>
                         </div>
                         ...
                         {myAuthorTours.length === 0 ? (
                             <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-gray-200">
                                 ...
                                 <p className="text-sm text-gray-400 mb-6">Start by creating your own unique itinerary to attract more clients.</p>
                                 <button onClick={() => openEditAuthorTour()} className="bg-indigo-50 text-indigo-600 font-bold px-6 py-3 rounded-xl hover:bg-indigo-100">Create My First Tour</button>
                             </div>
                         ) : (
                             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                 {myAuthorTours.map(t => (
                                     <div key={t.id} className="bg-white border rounded-xl overflow-hidden shadow-sm hover:shadow-md transition relative group">
                                         <div className="absolute top-2 right-2 z-10 flex gap-2">
                                             <button onClick={() => openEditAuthorTour(t)} className="bg-white/90 p-2 rounded-full shadow hover:bg-white text-indigo-600"><span className="sr-only">Edit</span>✎</button>
                                             <button onClick={() => { if(confirm('Delete tour?')) onDeleteTour(t.id) }} className="bg-white/90 p-2 rounded-full shadow hover:bg-red-50 text-red-500"><span className="sr-only">Delete</span>🗑️</button>
                                         </div>
                                         <img src={t.image} className="w-full h-40 object-cover" alt={t.titleEn} />
                                         <div className="p-4">
                                             <span className="bg-yellow-400 text-black text-[10px] font-bold px-2 py-0.5 rounded uppercase mb-2 inline-block">Author's Tour</span>
                                             <h4 className="font-bold text-lg leading-tight mb-2">{t.titleEn}</h4>
                                             <p className="text-xs text-gray-500 line-clamp-2 mb-3">{t.descriptionEn}</p>
                                             <div className="font-black text-emerald-600 text-lg">{t.price}</div>
                                         </div>
                                     </div>
                                 ))}
                             </div>
        ...
        {/* AUTHOR TOUR MODAL */}
        {isAuthorTourModalOpen && (
            <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 relative">
                    <button onClick={() => setIsAuthorTourModalOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600">✕</button>
                    <h3 className="text-xl font-bold mb-4">{editingAuthorTour.id?.includes('auth') ? 'Edit My Tour' : 'Create Author Tour'}</h3>
                    <form onSubmit={saveAuthorTour} className="space-y-4">
                        <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-100 text-sm text-yellow-800 mb-4">
                            <strong>Note:</strong> Author tours are special routes created by YOU. They will be highlighted in the search results.
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div><label className="text-xs font-bold text-gray-500">Title (EN)</label><input className="w-full border p-2 rounded" value={editingAuthorTour.titleEn || ''} onChange={e => setEditingAuthorTour({...editingAuthorTour, titleEn: e.target.value})} required /></div>
                            <div><label className="text-xs font-bold text-gray-500">Title (RU)</label><input className="w-full border p-2 rounded" value={editingAuthorTour.titleRu || ''} onChange={e => setEditingAuthorTour({...editingAuthorTour, titleRu: e.target.value})} required /></div>
                        </div>
                        <div><label className="text-xs font-bold text-gray-500">Description (EN)</label><textarea className="w-full border p-2 rounded" rows={2} value={editingAuthorTour.descriptionEn || ''} onChange={e => setEditingAuthorTour({...editingAuthorTour, descriptionEn: e.target.value})} /></div>
                        <div><label className="text-xs font-bold text-gray-500">Description (RU)</label><textarea className="w-full border p-2 rounded" rows={2} value={editingAuthorTour.descriptionRu || ''} onChange={e => setEditingAuthorTour({...editingAuthorTour, descriptionRu: e.target.value})} /></div>
                        
                        <div className="grid grid-cols-3 gap-4">
                            <div><label className="text-xs font-bold text-gray-500">Price Display (GEL)</label><input className="w-full border p-2 rounded" value={editingAuthorTour.price || ''} onChange={e => setEditingAuthorTour({...editingAuthorTour, price: e.target.value})} /></div>
                             {/* Base Price is hidden for author tours, we just use display price or logic */}
                            <div><label className="text-xs font-bold text-gray-500">Duration</label><input className="w-full border p-2 rounded" value={editingAuthorTour.duration || ''} onChange={e => setEditingAuthorTour({...editingAuthorTour, duration: e.target.value})} /></div>
                        </div>

                        <div>
                            <label className="text-xs font-bold text-gray-500 block mb-1">Image</label>
                            <input type="file" accept="image/*" className="mb-2 text-sm text-slate-500" onChange={handleAuthorTourImageUpload} />
                            {isUploadingTour && <span className="text-xs text-indigo-500 animate-pulse">Uploading...</span>}
                            {editingAuthorTour.image && <img src={editingAuthorTour.image} className="h-20 object-cover rounded mt-2" />}
                        </div>
                        
                        <div>
                            <label className="text-xs font-bold text-gray-500 block mb-1">Route Stops (Comma Separated)</label>
                            <input className="w-full border p-2 rounded text-xs" placeholder="Tbilisi, Signagi, Bodbe, Tbilisi" value={editingAuthorTour.routeStops?.join(', ') || ''} onChange={e => handleAuthorTourArrayInput('routeStops', e.target.value)} />
                        </div>
                        
                        <div>
                            <label className="text-xs font-bold text-gray-500 block mb-1">Highlights (EN - Comma Separated)</label>
                            <input className="w-full border p-2 rounded text-xs" value={editingAuthorTour.highlightsEn?.join(', ') || ''} onChange={e => handleAuthorTourArrayInput('highlightsEn', e.target.value)} />
                        </div>

                        <div className="flex justify-end gap-3 pt-4 border-t">
                            <button type="button" onClick={() => setIsAuthorTourModalOpen(false)} className="text-gray-500 font-bold">Cancel</button>
                            <button type="submit" className="bg-indigo-600 text-white px-6 py-2 rounded-lg font-bold">Save My Tour</button>
                        </div>
                    </form>
                </div>
            </div>
