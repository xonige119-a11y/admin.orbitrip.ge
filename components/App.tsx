...
const App = () => {
    ...
    // --- ROUTING INIT ---
    // Initialize view based on current URL path AND SUBDOMAIN
    const [currentView, setCurrentView] = useState(() => {
        const hostname = window.location.hostname;
        const path = window.location.pathname;

        // Check for admin subdomain
        if (hostname === 'admin.orbitrip.ge' || hostname === 'admin.localhost') {
            // Check if already logged in as driver
            const session = localStorage.getItem('orbitrip_driver_session');
            if (session) return 'DRIVER_DASHBOARD';
            return 'ADMIN_LOGIN';
        }

        if (path.startsWith('/admin')) return 'ADMIN_LOGIN';
        if (path.startsWith('/driver')) return 'DRIVER_DASHBOARD'; 
        if (path.startsWith('/tours')) return 'TOURS';
        if (path.startsWith('/blog')) return 'BLOG';
        if (path.startsWith('/drive-with-us')) return 'DRIVER_REGISTRATION';
        if (path.startsWith('/terms')) return 'LEGAL_TERMS';
        if (path.startsWith('/privacy')) return 'LEGAL_PRIVACY';
        return 'HOME';
    }); 
    ...
    useEffect(() => {
        const handlePopState = (event: PopStateEvent) => {
            if (event.state && event.state.view) {
                ...
                // Intelligent Fallback if history state is missing based on URL
                const hostname = window.location.hostname;
                if (hostname === 'admin.orbitrip.ge') {
                     // Stay in Admin/Driver flows on subdomain
                     const session = localStorage.getItem('orbitrip_driver_session');
                     setCurrentView(session ? 'DRIVER_DASHBOARD' : 'ADMIN_LOGIN');
                     return;
                }

                const path = window.location.pathname;
                if (path.startsWith('/admin')) setCurrentView('ADMIN_LOGIN');
                else if (path.startsWith('/tours')) setCurrentView('TOURS');
                else if (path.startsWith('/blog')) setCurrentView('BLOG');
                else if (currentView === 'BOOKING_PAGE') setCurrentView(searchParams ? 'SEARCH_RESULTS' : 'HOME');
                else if (currentView === 'DRIVER_PROFILE') setCurrentView('SEARCH_RESULTS');
                else if (currentView === 'SEARCH_RESULTS') { setCurrentView('HOME'); setSearchParams(null); }
    ...
    const handleLogout = () => {
        ...
        // If on admin subdomain, go to admin login, else home
        if (window.location.hostname === 'admin.orbitrip.ge') {
    ...
    const isSubdomainAdmin = window.location.hostname === 'admin.orbitrip.ge';
    ...
    // --- SUBDOMAIN MODE: ADMIN ONLY ---
    if (isSubdomainAdmin) {
        return (
            <ErrorBoundary language={language}>
                <SEO title="OrbiTrip Admin" description="Partner Portal" />
                    ...
                    <Suspense fallback={<PageLoader />}>
                        {(() => {
                            ...
                            if (currentView === 'ADMIN_DASHBOARD') {
                                ...
                                />;
                            } else if (currentView === 'DRIVER_DASHBOARD' && loggedInDriverId) {
                ...
                {(() => {
                    switch (currentView) {
                        ...
                        case 'DRIVER_DASHBOARD':
                            if (!loggedInDriverId) {
                                // If trying to access driver dashboard without session, redirect to login
                                setTimeout(() => navigateTo('ADMIN_LOGIN', '/admin'), 0);
                                return null;
                            }
                            return <DriverDashboard 
                                bookings={bookings} 
