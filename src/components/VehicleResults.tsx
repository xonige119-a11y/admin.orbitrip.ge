...
const getVehicleCategoryLabel = (type: string, isEn: boolean) => {
    switch (type) {
...
export const VehicleResults: React.FC<VehicleResultsProps> = ({ 
  ...
  const calculateTotalCost = (driver: Driver) => {
      const ROAD_FACTOR = 1.4; // Multiplier to convert Air Distance to Road Distance
      ...
      // Helper to find coords and metadata
      ...
      const driverLoc = findLocation(driverCity);
      const startLoc = findLocation(startName);
      const endLoc = findLocation(endName);
      ...
      // 4. Total Billable Distance (Round Trip)
      const totalBillableKm = approachKm + tripDistance + returnKm;

      // Driver Inputs: pricePerKm (Cost/km) and basePrice (Daily Salary)
      const dailySalary = driver.basePrice || 30; // Equivalent to 'Daily Salary' in new model
      const costPerKm = driver.pricePerKm || 0.3; // Equivalent to 'Cost per km' (e.g. 30 GEL / 100km = 0.3)
      
      // Smart Logic: Mountain Coefficient
      // If start or end point is flagged as mountainous, increase fuel/wear cost by 20%
      ...
      const fuelAndWearCost = totalBillableKm * costPerKm * mountainFactor;
      
      // 6. Net Total (Driver Payout)
      const netTotal = fuelAndWearCost + dailySalary;

      // 7. Final Price with Commission (Adding 13% on top)
      const commissionRate = 0.13; // 13% standard
      const finalPrice = Math.ceil(netTotal / (1 - commissionRate));
