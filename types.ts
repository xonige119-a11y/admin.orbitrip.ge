...
export interface SystemSettings {
  ...
  minTripPrice?: number; // Minimum price floor
  taxRate?: number; // NEW: Tax percentage
  currencySymbol?: string; // NEW: GEL, USD, etc.
  ...
  // System Control & Booking Rules
  maintenanceMode?: boolean; 
  bookingWindowDays?: number; // NEW: How far ahead can users book (e.g. 60 days)
  instantBookingEnabled?: boolean; // NEW: Skip pending status
  
  // Driver Policy
  autoApproveDrivers?: boolean; // NEW: Skip manual verification
  requireDocuments?: boolean; // NEW: Are docs mandatory?

  // Advanced Content & AI
  driverGuidelines?: string; // HTML/Markdown
  aiSystemPrompt?: string; // Custom prompt
  aiModelTemperature?: number; // NEW: 0.0 to 1.0 (Creativity)
  globalAlertMessage?: string; // Banner message
...
export interface LocationOption {
  ...
  nameEn: string;
  nameRu: string;
  type: 'AIRPORT' | 'CITY' | 'LANDMARK' | 'BORDER' | 'RESORT';
  lat: number;
  lng: number;
  isMountainous?: boolean; // NEW: Flag for price calculation
}
