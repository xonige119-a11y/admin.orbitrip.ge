...
-- ORBITRIP DATABASE SCHEMA (v2.3 - Advanced Configuration)
...
CREATE TABLE IF NOT EXISTS public.settings (
  ...
  driver_guidelines text, ai_system_prompt text, global_alert_message text,
  -- New Advanced Fields
  booking_window_days numeric default 60, instant_booking_enabled boolean default false,
  tax_rate numeric default 0, currency_symbol text default 'GEL',
  auto_approve_drivers boolean default false, require_documents boolean default true,
  ai_model_temperature numeric default 0.7
);
...
BEGIN 
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='settings' AND column_name='driver_guidelines') THEN 
    ALTER TABLE public.settings ADD COLUMN driver_guidelines text; 
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='settings' AND column_name='ai_system_prompt') THEN 
    ALTER TABLE public.settings ADD COLUMN ai_system_prompt text; 
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='settings' AND column_name='global_alert_message') THEN 
    ALTER TABLE public.settings ADD COLUMN global_alert_message text; 
  END IF;
  
  -- NEW FIELDS
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='settings' AND column_name='booking_window_days') THEN 
    ALTER TABLE public.settings ADD COLUMN booking_window_days numeric default 60; 
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='settings' AND column_name='instant_booking_enabled') THEN 
    ALTER TABLE public.settings ADD COLUMN instant_booking_enabled boolean default false; 
  END IF;
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='settings' AND column_name='currency_symbol') THEN 
    ALTER TABLE public.settings ADD COLUMN currency_symbol text default 'GEL'; 
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='settings' AND column_name='auto_approve_drivers') THEN 
    ALTER TABLE public.settings ADD COLUMN auto_approve_drivers boolean default false; 
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='settings' AND column_name='require_documents') THEN 
    ALTER TABLE public.settings ADD COLUMN require_documents boolean default true; 
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='settings' AND column_name='ai_model_temperature') THEN 
    ALTER TABLE public.settings ADD COLUMN ai_model_temperature numeric default 0.7; 
  END IF;
END $$;
